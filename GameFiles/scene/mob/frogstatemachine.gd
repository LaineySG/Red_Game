...
func _get_transition(_delta):
	...
	if parent.chase and (parent.navi.get_final_position().y < (parent.global_position.y - 10.0)) and !parent.dead and !parent.dying and state != states.happy and !parent.frozen:
		# if player is higher
		if parent.jumptimer.is_stopped():
			parent.jumptimer.start()
		if state != states.jump and parent.player_higher and !parent.webstuck:
