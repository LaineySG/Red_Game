...
func interactions(on):
	if on:
		...
		set_collision_mask_value(5,true) # makes it interactible withable.
		...
		set_collision_mask_value(5,false) # makes it uninteractible withable.
...
func attack():
	if !hypnotized:
		...
		frogtarget.hurt(0,damage/2,0,0)
		
func hypnoslow(caster):
...
func _on_hp_value_changed(value):
	if value <=0:
		...
		var coincount = rng.randi_range(1,4)
		for i in coincount:
