...
func hurt(damage,patiencedmg,DoT,MoT): # when hitbox is shot
	if damage > 0:
		...
		dmgnumspawn.settext(str(damage))
	...
	if patiencedmg > 0:
		...
		dmgnumspawn.position.x += locationmodx
		dmgnumspawn.position.y -= 50
		dmgnumspawn.settext(str(patiencedmg))
		dmgnumspawn.modulate = Color.DARK_SLATE_BLUE
		get_parent().add_child(dmgnumspawn)
	...
	if !dying and !dead:	
		if damage > 0:
			...
			if Game.current_effects.has("Freeze-Ray (Toygun)"):
				...
				frosty.tween_property(get_node("AnimatedSprite2D"),"modulate", Color.BLUE,0.2)
				await get_tree().create_timer(2.0).timeout
				var returner = get_tree().create_tween()
				returner.tween_property(get_node("AnimatedSprite2D"),"modulate", Color.WHITE, 0.5)
				speed += 30
				frosted += 1
			if Game.current_effects.has("Shrink-Ray (Toygun)"):
				...
				scale.y *= 0.85
				if attackdamage > 1:
					attackdamage *= 4.0 / 5.0
				else:
					attackdamage = 1
			var rng = RandomNumberGenerator.new()
			var randchance = rng.randf()
...
func _on_hp_value_changed(value):
	if value <=0:
		...
		var kill = wtf.instantiate()
		var locationmodx = rng.randi_range(-15,15)
		kill.position = player.global_position
		kill.position.x += locationmodx
		kill.position.y -= 15
		kill.settext("... :)")
		kill.modulate = Color(1,0,0,1)
