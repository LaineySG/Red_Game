...
func _ready():
	...
	Utils.unpausegame()
	if Game.tutorial_area2count == 0:
		#get_node("cutscene player").play("tutorial_area_1_cutscene_1")
		...
		get_node("walljump").visible = false
		get_node("cantgothere").text = "Here goes nothing."
		get_node("buttonarea").visible = true
		get_node("buttonarea").locked = false
		get_node("buttonarea").add_to_group("interact")
